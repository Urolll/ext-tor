...
Each pair from the cartesian product $V \times W$ of the form $(v, w)$, where $v \in V$ and $w \in W$, gets mapped to $v \otimes w \in V \otimes W$. These elements are called the *elementary tensors* of $V \otimes W$ and they span the entire space.
...
$$
\operatorname{Ext}_R^n(M, N) := H^n(\text{Hom}_R(P_\bullet, N))
$$
